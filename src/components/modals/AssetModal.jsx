      ...
      {isImmo && (
        <div style={{ background:T.surfaceHigh, border:"1px solid "+T.border, borderRadius:8, padding:12, marginBottom:12 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8 }}>
            <Inp label="Kaltmiete/Mo." value={f.monthlyRent} onChange={v => setF(p => ({ ...p, monthlyRent:v }))} type="number" placeholder={String(IMMO_CF_GROSS)} T={T} />
            <Inp label="Hausgeld/Mo." value={f.hausgeld} onChange={v => setF(p => ({ ...p, hausgeld:v }))} type="number" placeholder={String(IMMO_HAUSGELD)} T={T} />
            <Inp label="Grundsteuer/Mo." value={f.grundsteuer} onChange={v => setF(p => ({ ...p, grundsteuer:v }))} type="number" placeholder={String(IMMO_GRUNDSTEUER)} T={T} />
          </div>
          <div style={{ fontSize:9, color:immoNetCF>=0?T.green:T.red, marginTop:6 }}>
            ...
            {hasDebt && <span style={{ color:immoNetCF>=0?T.green:T.red }}> | nach Annuitat: {full(immoNetCF)}/Mo.</span>}
          </div>
        </div>
      )}
